    ...
    <template>
      <div style="width: 80%;margin:0px;position:relative"></div>
        <div  v-if="$store.state.src != null" >
          ...
          <span style="position:absolute;bottom:35%;right:15%" v-if="$store.state.musicList.length!=0&&kind==1">待播：{{$store.state.musicList.length-1}}</span> 
      ...
      <div  style="margin:0px" v-else>当前暂无音乐播放</div>
    </template>
</el-drawer>
