<template>
   ...
   align="center" active-text-color="white">
...
<el-drawer id="mydrawer" :visible.sync="drawer" direction="btt" :modal="false" @mouseleave="out()"
  manual="false" :size="size">
    <template slot="title">
      <div style="height:10px" @click="size='100%'" v-if="size!='100%'"><i class="el-icon-arrow-up"></i></div>
      <div style="height:10px" @click="size='25%'" v-else><i class="el-icon-arrow-down"></i></div>
    </template>
    <template>
      <div style="width: 80%;margin:0px;position:relative"></div>
        <div  v-if="$store.state.src != null" >
          <audio id="audio1" controls autoplay style="width: 80%;position:absolute;bottom:15%;right:10%" :src="$store.state.src">
          </audio> 
            <el-carousel :interval="4000" type="card" height="400px"  style="width: 100%;position:absolute;top:12%;right:10%" v-if="size=='100%'">
          ...
          <span style="position:absolute;bottom:10%;right:15%" v-if="$store.state.musicList.length!=0&&kind==1">待播：{{$store.state.musicList.length-1}}</span> 
            <el-tooltip style="position:absolute;bottom:15%;right:6%"  class="item" effect="dark" content="按列表播放" placement="right" v-if="kind==1">
              <el-button icon="el-icon-refresh-right"  @click="kind1()" circle></el-button>
            </el-tooltip>
            <el-tooltip style="position:absolute;bottom:15%;right:6%"  class="item" effect="dark" content="单曲循环" placement="right" v-if="kind==2">
              <el-button   icon="el-icon-refresh" @click="kind2()" circle></el-button>
            </el-tooltip>
            <el-tooltip style="position:absolute;bottom:15%;right:6%"  class="item" effect="dark" content="播完暂停" placement="right" v-if="kind==3">
      ...
      <div  style="margin:0px" v-else>请继续添加音乐哦~</div>
    </template>
</el-drawer>
