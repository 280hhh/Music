      ...
      <div style="width: 80%;margin:0px;position:relative"></div>
        <div  v-if="$store.state.src != null" >
          ...
          <span style="position:absolute;bottom:10%;right:15%" v-if="$store.state.musicList.length!=0&&kind==1">待播：{{$store.state.musicList.length-1}}</span> 
            <el-tooltip style="position:absolute;bottom:36%;right:6%"  class="item" effect="dark" content="按列表播放" placement="right" v-if="kind==1">
              <el-button icon="el-icon-refresh-right"  @click="kind1()" circle></el-button>
            </el-tooltip>
            <el-tooltip style="position:absolute;bottom:36%;right:6%"  class="item" effect="dark" content="单曲循环" placement="right" v-if="kind==2">
              <el-button   icon="el-icon-refresh" @click="kind2()" circle></el-button>
            </el-tooltip>
            <el-tooltip style="position:absolute;bottom:36%;right:6%"  class="item" effect="dark" content="播完暂停" placement="right" v-if="kind==3">
